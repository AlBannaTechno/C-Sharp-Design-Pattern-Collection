...
{
    namespace S_3_Singleton_TestabilityWithInterfaceSolution
    {
        ...
        class ConfigurableSingletonDatabaseUser
        {
            private readonly IDatabase _database;

            public ConfigurableSingletonDatabaseUser(IDatabase database)
            {
                _database = database;
            }
            public int GetSumOfAllRooms(IEnumerable<string> rooms)
            {
                ...
                {
                    result += _database.GetUserRoomNumber(room);
                }
                return result;
            }
        }
